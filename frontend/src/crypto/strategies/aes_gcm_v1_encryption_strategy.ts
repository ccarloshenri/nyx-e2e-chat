...
export class AesGcmV1EncryptionStrategy implements IEncryptionStrategy {
  getEncryptionType(): EncryptionType {
    return EncryptionType.AES_GCM_V1;
  }
  ...
  async encrypt(input: EncryptMessageInput): Promise<EncryptedMessagePayload> {
    ...
    return {
      ...
      encrypted_message_key: input.encryptedMessageKey,
  ...
  async decrypt(input: DecryptMessageInput): Promise<string> {
    if (input.payload.encryption_type !== this.getEncryptionType()) {
      throw new Error("AES_GCM_V1 strategy cannot decrypt a different encryption type.");
    }
